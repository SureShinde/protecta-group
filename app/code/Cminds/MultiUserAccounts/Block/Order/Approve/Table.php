...
class Table extends AbstractCart
{
    ...
    public function getQuotes()
    {
        ...
        if (!$this->quotes) {
            ...
            // If shared session is enabled, we need to update customer data
            if (true === $this->moduleConfig->isSharedSessionEnabled()) {
                foreach ($quoteCollection as $key => $quote) {
                    if (null === $quote->getSubaccountId()) {
                        continue;
                    }

                    $subaccountData = $this->subaccountTransportRepository->getByCustomerId($quote->getSubaccountId());
                    // update first name, last name, email                        
                    $quote->setCustomerFirstname($subaccountData->getFirstName());
                    $quote->setCustomerLastname($subaccountData->getLastName());
                    $quote->setCustomerEmail($subaccountData->getEmail());
                    // replace item in the collection
                    $quoteCollection->removeItemByKey($key);
                    $quoteCollection->addItem($quote);
                }
            }
