...
class SubmitBefore implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
         ...
         * for shared session and force use parent account details.
         *
         */
        $quote = $observer->getQuote();
        $parentCustomerObject = $this->customerRepository->getById(
            $subaccountTransportDataObject->getParentCustomerId()
        );
        $subaccountCustomerObject = $this->customerRepository->getById(
            $subaccountTransportDataObject->getCustomerId()
        );

        if ($this->moduleConfig->isSharedSessionEnabled() === true
            && $this->moduleConfig->isForceUseParentAccountDetailsForOrderEnabled() === false
        ) {
            // Use subaccount details.
            $originalCustomerId = $quote->getCustomerId();
            $quote->updateCustomerData($subaccountCustomerObject);
            $updatedCustomerId = $quote->getCustomerId();

            $this->registry->register(
                self::CMINDS_MULTIUSERACCOUNTS_CHANGE_TEMP_USER_ID,
                [
                    'original_user_id' => $originalCustomerId,
                    'updated_user_id' => $updatedCustomerId
                ]
            );

            $quote->save();
        } elseif ($this->moduleConfig->isSharedSessionEnabled() === true
            && $this->moduleConfig->isForceUseParentAccountDetailsForOrderEnabled() === true
        ) {
            // Use parent details.
            $originalCustomerId = $quote->getCustomerId();
            $quote->updateCustomerData($parentCustomerObject);
            $updatedCustomerId = $quote->getCustomerId();
