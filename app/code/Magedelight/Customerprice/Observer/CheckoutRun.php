...
class CheckoutRun implements ObserverInterface
{
    protected $logger;
    ...
    \Psr\Log\LoggerInterface $logger, \Magedelight\Customerprice\Helper\Data $helper, \Magento\Checkout\Model\Session $session, \Magento\Catalog\Model\Product\Type\Price $priceModel, \Magento\Customer\Model\Session $customerSession
    ) {
        $this->logger = $logger;
        $this->helper = $helper;
    ...
    {
        $curaction = $observer->getEvent()->getData('request')->getFullActionName();
        if ($curaction == 'checkout_cart_index') {
                ...
                foreach ($cartItems as $item) {
                    ...
                    if ($finalPrice) {
                        if (gettype($finalPrice) == 'object') {
                            ...
                            $item->getProduct()->setPrice($finalPrice);
