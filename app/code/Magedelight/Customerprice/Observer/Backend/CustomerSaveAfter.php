...
class CustomerSaveAfter implements ObserverInterface {
    ...
    \Magento\Framework\App\Request\Http $request, \Psr\Log\LoggerInterface $logger, \Magedelight\Customerprice\Helper\Data $helper, \Magento\Checkout\Model\Session $session, \Magento\Catalog\Model\Product\Type\Price $priceModel, \Magento\Customer\Model\CustomerFactory $customerFactory, \Magento\Customer\Model\Session $customerSession
    ) {
                ...
                if (!is_null($discount) && $discount->getId()) {
                    ...
                    if (is_numeric($discountValue['discount'])) {
                        $discountModel = $this->_objectManager->create('Magedelight\Customerprice\Model\Discount')
                                ->setData(array('customer_id' => $customer_id, 'value' => $discountValue['discount']));
                        try {
                            $discountModel->save();
                        } catch (\Exception $e) {
