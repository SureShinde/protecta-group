    ...
    public function execute() {
        ...
        if (isset($_FILES['image']) && isset($_FILES['image']['name']) && strlen($_FILES['image']['name'])) {
			try{
				...
				$result = $uploader->save($mediaDirectory->getAbsolutePath('homeimage'));
					if($result['error']==0) {
						$data['image'] = 'homeimage' . $result['file'];
					}
			} catch (\Exception $e) {
            ...
            try {
                $model->save();
                $this->messageManager->addSuccess(__('The Home Image has been saved.'));
                ...
                $this->messageManager->addException($e, __('Something went wrong while saving the Home Image.'));
            }
