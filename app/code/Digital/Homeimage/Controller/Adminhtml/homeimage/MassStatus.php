...
class MassStatus extends \Magento\Backend\App\Action {
    ...
    public function execute() {
        $itemIds = $this->getRequest()->getParam('homeimage');
        if (!is_array($itemIds) || empty($itemIds)) {
			...
			try {
                ...
                foreach ($itemIds as $postId) {
                    $post = $this->_objectManager->create('Digital\Homeimage\Model\Homeimage')->load($postId);
        ...
        return $this->resultRedirectFactory->create()->setPath('homeimage/*/index');
    }
}