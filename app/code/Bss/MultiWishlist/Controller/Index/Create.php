...
class Create extends Action
{
    ...
    public function execute()
    {
        ...
        if ($result->getId() || strtolower($name) == 'main') {
            if (!isset($param['ajax'])) {
                $this->messageManager->addErrorMessage(__('Already exist a projectlist. Please choose a different name.'));
            } else {
                $response['error'] = "<div class='message-success success message'>
                    <div data-bind='html: message.text'>".
                    __('Already exist a project list. Please choose a different name.').".</div></div>";
                $response['html'] = '';
            }
        } else {
            try {
                ...
                if (!isset($param['ajax'])) {
                    $this->messageManager->addSuccessMessage(__('Successfully saved the project list.'));
                } else {
                    $response['success'] = "<div class='message-success success message'>
                        <div data-bind='html: message.text'>".__('Successfully saved the project list.').".</div></div>";
