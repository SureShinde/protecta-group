...
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
		...
		if (version_compare($context->getVersion(), '2.6.0', '<')) {
            if ($installer->tableExists('mageplaza_blog_post')) {
                if (!$connection->tableColumnExists($installer->getTable('mageplaza_blog_post'), 'new_author_name')) {
                    $connection->addColumn($installer->getTable('mageplaza_blog_post'), 'new_author_name', [
                        'type'    => Table::TYPE_TEXT, null, ['unsigned' => true, 'nullable' => true],
                        'comment' => 'Author Name',
                    ]);
                }
