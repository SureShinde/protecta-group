...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This resource no longer exists.'));
        ...
        $this->_coreRegistry->register('downloadcatalogue', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Resource') : __('New Resource'),
            $id ? __('Edit Resource') : __('New Resource')
        );
        $resultPage->getConfig()->getTitle()->prepend($id ? __('Edit Resource') : __('New Resource'));
        /* $resultPage->getConfig()->getTitle()->prepend(__('Downloadcatalogue'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Download Catalogue')); */
        return $resultPage;
    }
}
