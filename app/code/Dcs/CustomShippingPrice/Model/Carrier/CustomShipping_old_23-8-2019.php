...
class CustomShipping extends AbstractCarrier implements CarrierInterface
{
	...
	public function getTotalVolume($quote)
	{		
		$volume_total = 0;		
	...
	public function getCheckProductAvailableInStore($region_name,$quote)
	{		
		$status_array = array();
		$items = $quote->getAllItems();		
		foreach($items as $item)
	...
	public function getAllProductProfile($quote)
	{		
		$profile_array = array();		
		$items = $quote->getAllItems();		
		foreach($items as $item)
	...
	public function showDeliveryOptions($results_final)
	{
		...
		{			
			if (!empty($result_item) && $result_item['price'] >= 0)
			{
				...
				$method->setMethod('dcsshipping_delivery_' . $result_item['id']);
	...
	public function getFinalQueryData($country_name,$region_name,$zone_number,$postcode,$weight_total,$volume_total,$profile_name)
	{
		...
		$query_final = "SELECT id,price,delivery_type FROM `digital_matrixrate` 
			...
			{											
					...
					foreach($zone_row_array as $zone_item)
					{
						...
						if($region_name)
						{
							...
							$all_prod_status = $this->getCheckProductAvailableInStore($region_name,$quote);				
							...
							else if(in_array("1", $all_prod_status))   
							{
								...
								if(count(array_unique($all_prod_status)) === 1)  /// All Cart QTY Match with Region Store...go ahead for check weight,volume, profile etc....
								{
									...
									$volume_total = $this->getTotalVolume($quote);
									$all_profile_array = $this->getAllProductProfile($quote);
